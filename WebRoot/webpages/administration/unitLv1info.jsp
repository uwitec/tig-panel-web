  ...
  <head>    
    ...
    <script type="text/javascript">
		...
		var createConditionPanel = function(){
				conditionPanel = new SelfToolBarPanel('queryConditionPanel', '一级单位管理', 120, 1,
				[
					{rowIndex:0, field:{xtype:'textfield', name:'nodename', fieldLabel:'单位名称'}},
					{rowIndex:0, field:{xtype:'hidden', name:'nodelevel', value:'2'}}
				],
				[
		...
		function loadPage(){
			var queryliteUnitName = new PageQueryLite('queryliteUnitName',
					'<%=basePath%>/BasePackage/privilegequery_queryUnitInfo?nodelevel=1',
			...
			addwindow = new SelfFormWindow('recordAddWindow', '添加', 250, 340, 200, 1,
				[	
					{colIndex:0, field:{xtype:'numberfield', name:'nodeid', fieldLabel:'*一级单位编码', allowBlank:false, blankText:'必须填写'}},
					{colIndex:0, field:{xtype:'textfield', name:'nodename', fieldLabel:'*一级单位名称',allowBlank:false, blankText:'必须填写',allowNegative:false, allowDecimals:false}},
					{colIndex:0, field:queryliteUnitName.getTextField()},
					{colIndex:0, field:queryliteUnitName.getTextHiddenField()}
				],
				[
			...
			var queryliteUnitName1 = new PageQueryLite('queryliteUnitName1',
					...
					['parentnodeid']);
					queryliteUnitName1.setValue(logonUser.unit.nodeid);
					queryliteUnitName1.setText(logonUser.unit.nodename);
		...
		function onaddclicked(){
			...
			requestAjax('<%=basePath%>/admin/systemunit_addunit?param=2',submitData,function(sRet){
		...
		function oneditclicked(){
			var submitData = editwindow.getFields();
				requestAjax('<%=basePath%>/admin/systemunit_editunit?param=2',submitData,function(sRet){
